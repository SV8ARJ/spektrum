class HackRFspektrum implements SpektrumInterface {
    ...
    void setFrequencyRange(int startFreq, int stopFreq, int binStep, double tmpCrop) {
		...
		glb_cmdFrequencyRange = "-f " + startFreq/1000000 + ":" + stopFreq/1000000 + " -w " + tmpBinStep;
    ...
    @Override
    int[] getGains() {
        // Returning a dummy array for HackRFspektrum for now
	...
	public void runHackRFSweepInBackground() {
		// Initialize an ArrayList to store the output lines
		  ...
		  try {
			// Define the command to run hackrf_sweep.exe (adjust the path if necessary)
			...
			// "hackrf_sweep.exe -f 100:500 -w 10000";  // Adjust the command as needed
